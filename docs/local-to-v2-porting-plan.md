...
Estimated total effort: ~0 L + ~4 M + ~6 S ≈ 1.5–2.5 focused weeks (was 3–4 before the `upstream/channels` discovery; further trimmed 2026-05-17 when step 3 turned out to be pre-landed via `send_file`, then again when step 4 landed).

**Integration branch:**

- `local-v2` (off `upstream/channels`) — currently at step 4's tip (`517c9af`). Checked out in worktree `/tmp/nanoclaw-signal-pr/`. Live install on `local` continues running v1; switch over once `local-v2` reaches feature parity. Step 3 needed no code commits (pre-landed via `send_file`); steps 2 and 4 landed as code commits on `local-v2`. Next active integration step is #5 (session rotation).
