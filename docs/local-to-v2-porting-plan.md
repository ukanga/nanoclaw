...
Estimated total effort: ~0 L + ~3 M + ~6 S ≈ 1.5–2 focused weeks (was 3–4 before the `upstream/channels` discovery; further trimmed 2026-05-17 when step 3 turned out to be pre-landed via `send_file`, then again when step 4 landed; trimmed again when step 5 landed as a smaller container-side change than the plan predicted).

**Integration branch:**

- `local-v2` (off `upstream/channels`) — currently at step 5's tip (`2ea0795`). Checked out in worktree `/tmp/nanoclaw-signal-pr/`. Live install on `local` continues running v1; switch over once `local-v2` reaches feature parity. Step 3 needed no code commits (pre-landed via `send_file`); steps 2, 4, and 5 landed as code commits on `local-v2`. Next active integration step is #6 (session commands).
