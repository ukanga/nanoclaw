...
Estimated total effort: ~0 L + ~3 M + ~3 S ≈ 1 focused week (was 3–4 before the `upstream/channels` discovery; further trimmed 2026-05-17 when step 3 turned out to be pre-landed via `send_file`, then again when step 4 landed; trimmed again when step 5 landed as a smaller container-side change than the plan predicted; trimmed once more when step 6 turned out to be pre-landed via `src/command-gate.ts`; trimmed again when step 7 landed as a real port on top of `messages_in` rather than dropping in via a v2 mechanism that turned out not to exist; trimmed once more when step 8 landed as a mostly-mechanical vendor with a small surface-refresh on `capabilities` + `status`; on step 9 the S replay landed but discovery that upstream's claw is itself v1-coded added a new M step 13 for the v2-runner port).

**Integration branch:**

- `local-v2` (off `upstream/channels`) — currently at step 9's tip (`89be50b`). Checked out in worktree `/tmp/nanoclaw-signal-pr/`. Live install on `local` continues running v1; switch over once `local-v2` reaches feature parity. Steps 3 and 6 needed no code commits (pre-landed upstream via `send_file` and `command-gate.ts` respectively); steps 2, 4, 5, 7, 8, and 9 landed as code commits on `local-v2`. Next active integration step is #10 (helper scripts).
...
- [ ] **2.5 `claw` (v2 runner port)** — TODO. Before writing code, re-read `scripts/init-cli-agent.ts` and `scripts/test-v2-agent.ts` for the v2-native CLI patterns (create-user / create-agent-group / messages_in writer / messages_out reader). Re-check whether step 10 builds an `ncl` CLI claw can wrap before sizing — if yes, this step collapses S; if no, it stays M.
